...
class Application():
  def __call__(self, environ, start_response):
    ...
    i = 0
    while i < len(APJP.APJP.APJP_REMOTE_HTTP_SERVER_RESPONSE_PROPERTY_KEY):
      if APJP.APJP.APJP_REMOTE_HTTP_SERVER_RESPONSE_PROPERTY_KEY[i] != '':
        http_response_headers.append((APJP.APJP.APJP_REMOTE_HTTP_SERVER_RESPONSE_PROPERTY_KEY[i], APJP.APJP.APJP_REMOTE_HTTP_SERVER_RESPONSE_PROPERTY_VALUE[i]))
      i = i + 1
...
class ApplicationIterator():
  ...
  def __iter__(self):
    ...
    socket1.connect((http_request_address, http_request_port))
    socket1.send(http_request_header)
    ...
    while True:
      ...
      if buffer_length == 0:
        break
      socket1.send(cipher.decrypt(buffer))
    
    cipher = Crypto.Cipher.ARC4.new(APJP.APJP.APJP_KEY)
    
    while True:
      buffer = socket1.recv(5120)
      buffer_length = len(buffer)
      if buffer_length == 0:
        break
