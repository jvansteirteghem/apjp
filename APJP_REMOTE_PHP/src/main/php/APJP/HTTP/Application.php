...
class APJP_HTTP_Application
{
	public function execute()
	{
		...
		$handle2 = fsockopen($http_request_address, $http_request_port);
		...
		for($i = 0; $i < count(APJP_APJP::$APJP_REMOTE_HTTP_SERVER_RESPONSE_PROPERTY_KEY); $i = $i + 1)
		{
			if(APJP_APJP::$APJP_REMOTE_HTTP_SERVER_RESPONSE_PROPERTY_KEY[$i] != '')
			{
				header(APJP_APJP::$APJP_REMOTE_HTTP_SERVER_RESPONSE_PROPERTY_KEY[$i] . ': ' . APJP_APJP::$APJP_REMOTE_HTTP_SERVER_RESPONSE_PROPERTY_VALUE[$i]);
			}
		}
