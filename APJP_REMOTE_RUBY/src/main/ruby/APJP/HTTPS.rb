...
class HTTPS
  def call(env)
    ...
    for i in 0 .. $APJP_REMOTE_HTTPS_SERVER_RESPONSE_PROPERTY_KEY.length - 1
      if $APJP_REMOTE_HTTPS_SERVER_RESPONSE_PROPERTY_KEY[i] != "" then
        http_response_headers[$APJP_REMOTE_HTTPS_SERVER_RESPONSE_PROPERTY_KEY[i]] = $APJP_REMOTE_HTTPS_SERVER_RESPONSE_PROPERTY_VALUE[i]
      end
    end
    
    response = Rack::Response.new(HTTPSEnumerable.new(request), 200, http_response_headers)
    return response
  end
end

class HTTPSEnumerable
  include Enumerable
