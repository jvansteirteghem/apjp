  ...
  module HTTPS
    class Application
      def call(env)
        ...
        for i in 0 .. APJP.APJP_REMOTE_HTTPS_SERVER_RESPONSE_PROPERTY_KEY.length - 1
          if APJP.APJP_REMOTE_HTTPS_SERVER_RESPONSE_PROPERTY_KEY[i] != "" then
            http_response_headers[APJP.APJP_REMOTE_HTTPS_SERVER_RESPONSE_PROPERTY_KEY[i]] = APJP.APJP_REMOTE_HTTPS_SERVER_RESPONSE_PROPERTY_VALUE[i]
          end
        end
    ...
    class ApplicationIterator
      ...
      def each
        ...
        ssl_socket.write(http_request_header)
        
        buffer = @request.body.read(5120)
        while buffer != nil
          ssl_socket.write(cipher.update(buffer))
          
          buffer = @request.body.read(5120)
        end
        
        ssl_socket.write(cipher.final())
